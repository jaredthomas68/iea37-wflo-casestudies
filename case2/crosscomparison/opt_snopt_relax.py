...
if __name__ == "__main__":
    ...
    layout_directory = "../../iea37-cs2-crosscomp/"
    layout_file =  "iea37-par%i-cc%i.yaml" % (layout_number, layout_number)
    ...
    turbineX = np.asarray(layout_data['definitions']['position']['items']['xc'])
    turbineY = np.asarray(layout_data['definitions']['position']['items']['yc'])
        ...
        for expansion_factor, i in zip(expansion_factors, np.arange(0, expansion_factors.size)):  # best so far
            ...
            if prob.root.comm.rank == 0:
                ...
                if save_locations:
                    ...
                    with open(layout_directory + "iea37-par4-cc4.yaml", 'r') as f:
                    ...
                    loaded_yaml['definitions']['plant_energy']['properties']['annual_energy_production'][
                        'binned'] = np.matrix.tolist(np.matrix(prob['dirPowers'] * windFrequencies * 24.*365.* 1E-6))
                    loaded_yaml['definitions']['plant_energy']['properties']['annual_energy_production']['default'] = \
                    float(prob['AEP']*1E-6)


                    with open(
                            output_directory + 'iea37-par4-cc%i.yaml' % (layout_number),
                            "w") as f:
